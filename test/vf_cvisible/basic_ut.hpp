...
public:
    ...
    void randomSeed ()
    {
        ...
        vf_utils::time_spec_t now = vf_utils::time_spec_t::get_system_time ();
        uint32 seed = now.get_frac_secs ();
        srandom (seed);
    }
    ...
    static std::string makeTmpName(const char *pathFormat, const char* baseName )
    {
        ...
        double secs = vf_utils::time_spec_t::get_system_time ().get_full_secs ();
