...
public:
  ...
  // access the device, for example to chk the error status
...
void dump(dump_context&, const std::vector<bool>& data) throw(persistent_dump_failed);
void restore(restore_context&, std::vector<bool>& data) throw(persistent_restore_failed);
