/* 
   ...
   $Id: rc_tree.hh 6329 2009-01-29 05:39:00Z arman $
...
class tree {
	...
	public:
		...
		// insert node (with children) pointed to by subtree as previous sibling of node pointed to by position
		// template<typename iter> iter insert_subtree(sibling_iterator position, iter subtree);
...
//		void     merge(const iterator_base& position, const iterator_base& other, bool duplicate_leaves=false);
		...
		bool     equal(const iterator_base& one, const iterator_base& two, const iterator_base& three, 
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T, tree_node_allocator>::insert(sibling_iterator position, const T& x)
	{
	tree_node* tmp = alloc_.allocate(1,0);
	constructor(&tmp->data, x);
	tmp->first_child=0;
...
template <class T, class tree_node_allocator>
template <class BinaryPredicate>
bool tree<T, tree_node_allocator>::equal(const iterator_base& one_, const iterator_base& two, const iterator_base& three_, BinaryPredicate fun) const
	{
