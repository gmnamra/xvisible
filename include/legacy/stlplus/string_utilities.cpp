...
otext& print(otext& str, const std::vector<bool>& values)
{
  for (size_t i = 0; i < values.size(); i++)
    str << (values[i] ? '1' : '0');
  return str;
...
std::string translate(const std::string& input, const std::string& from_set, const std::string& to_set)
{
  ...
  {
    char ch = input[i];
    // chk to see if the character is in the from set
...
std::string display_time(time_t seconds)
{
  ...
  if (!result.empty())
    result += to_string((unsigned)seconds, 10, radix_none, 2);
  else
  {
    result += to_string((unsigned)seconds, 10, radix_none, 1);
    result += "s";
  }
  return result;
}
