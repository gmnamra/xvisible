...
public:
  rc1dPeak() {};
  rc1dPeak(T v,const int32 l) : mLocation(l), mValue(v) {};

  const int32 location () const
  {
    return mLocation;
  }
...
template<class P>
void rf1DPeakDetect(const vector<P>& signal, vector<uint32>& peakBin,
		    ...
		    const P hysteresisPct, bool DEBUG_PD = false)
{
   ...
   */
  int32 pPeak = 0, pValley = 0, min = 0, max = 0;
  ...
  for (int32 i = 1; i < (int32)signal.size(); i++) {
    if (DEBUG_PD) printf("signal[%d] = %f", i, signal[i]);

    if ((pPeak >= 0) && signal[i] < peakThresh) {
      // Found peak
      peakBin.push_back((uint32)pPeak);
      P peakValue;
      peakLoc.push_back(rfInterpolatePeak(signal, (uint32)pPeak, &peakValue));
...
class rcEdgeFilter1D
{
 public:
   ...
   */
  void filter(const vector<int32>& filter);
   ...
   * filter being used.
   */
  void genPeaks(const rcWindow& img,
   ...
   * oneCnt must be >= 1.
   */
  static void genFilter(vector<int32>& filter, 
			const uint32 oneCnt, const uint32 zeroCnt);

 private:
  vector<int32> _filter;
  vector<int32> _filteredImg;
  double _stdDev;
  uint32 _min, _max;
};
...
void rfCtfDiff1d (deque<S>& src, int32 halfWidth, deque<D>& dst, T& dum, bool direction)
{
  int32 kernelWidth = halfWidth * 2 + 1; // one bin of leniency
  ...
  int32 rightSize_ (halfWidth);
  int32 leftSize_ (halfWidth);
