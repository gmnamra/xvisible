...
#ifndef _vfVideoCache_H_
#define _vfVideoCache_H_
...
/* vfVideoCache - Implements a read-only cache of video frames stored
...
class vfVideoCache
{
 ...
 public:
  /* vfVideoCacheCtor - Opens file and initializes data structures. If
   ...
   *
   */
  static vfVideoCache* vfVideoCacheCtor(const std::string fileName,
  ...
  /* vfVideoCacheDtor - Removes pointer to this cache from the map of
   ...
   * treated as noops.
   */
  static void vfVideoCacheDtor(vfVideoCache* cache);

  /* Returns the frame at location index within the movie (zero
   * base).
   */
  vfVideoCacheStatus getFrame(uint32 frameIndex,
			      rcSharedFrameBufPtr& frameBuf,
			      vfVideoCacheError* error = 0,
			      bool locked = true);

  /* Returns the frame at timestamp time. must be exact match.
   */
  vfVideoCacheStatus getFrame(const rcTimestamp& time,
			      rcSharedFrameBufPtr& frameBuf,
			      vfVideoCacheError* error = 0,
			      bool locked = true);
  ...
  vfVideoCacheStatus closestTimestamp(const rcTimestamp& goalTime,
				      rcTimestamp& match,
				      vfVideoCacheError* error = 0);
  ...
  vfVideoCacheStatus nextTimestamp(const rcTimestamp& goalTime,
				   rcTimestamp& match,
				   vfVideoCacheError* error = 0);
  // Returns closest timestamp < goalTime. If no such timestamp can be
  // found, the return value is eVideoCacheStatusError, and
  // eVideoCacheErrorNoSuchFrame is returned in error.
  vfVideoCacheStatus prevTimestamp(const rcTimestamp& goalTime,
				   rcTimestamp& match,
				   vfVideoCacheError* error = 0);
  // Returns the timestamp of the first frame in the movie.
  vfVideoCacheStatus firstTimestamp(rcTimestamp& match,
				    vfVideoCacheError* error = 0);
  // Returns the timestamp of the last frame in the movie.
  vfVideoCacheStatus lastTimestamp(rcTimestamp& match,
				   vfVideoCacheError* error = 0);
  ...
  // returned in error.
  vfVideoCacheStatus frameIndexToTimestamp(uint32 frameIndex,
					   rcTimestamp& match,
					   vfVideoCacheError* error = 0);
  ...
  // returned in error.
  vfVideoCacheStatus timestampToFrameIndex(const rcTimestamp& timestamp,
					   uint32& match,
					   vfVideoCacheError* error = 0);

  /* Get error that caused video cache to become invalid.
   */
  vfVideoCacheError getFatalError() const;
   ...
   */
  static std::string getErrorString(vfVideoCacheError error);

 private:

  /* Class to be used by vfVideoCache objects to create a thread that
   * will be used to do prefetches of video frames.
   *
   * Creator should call requestSeppuku() to gracefully shut down the
   * thread.
   */
  class vfVideoCachePrefetchUnit : public rcThread
  {
  public:
    vfVideoCachePrefetchUnit(vfVideoCache& cacheCtrl);
    
    virtual ~vfVideoCachePrefetchUnit();
   ...
   * description of vfVideoCacheCtor() for more details.
   */
  vfVideoCache(const std::string fileName, uint32 cacheSize, bool getTOC,
               bool verbose, bool prefetch, uint32 maxMemory,
               rcProgressIndicator* pIndicator);

  virtual ~vfVideoCache();

  /* finishSetup - Inserts newly created video cache into the global
   * list of active caches and gives it a unique ID.
   */
  static vfVideoCache* finishSetup(vfVideoCache* cacheP);

  /* vfVideoCacheUTCtor - Allow UT clients to create a video cache
   ...
   * "frames" will fail.
   */
  static vfVideoCache* vfVideoCacheUTCtor(const vector<rcTimestamp>& frameTimes);
  vfVideoCache(const vector<rcTimestamp>& frameTimes);

  void setError(vfVideoCacheError error);

  vfVideoCacheStatus internalGetFrame(uint32 frameIndex,
				      rcSharedFrameBufPtr& frameBuf,
				      vfVideoCacheError* error,
				      const uint32 dToken);

  vfVideoCacheStatus cacheAlloc(uint32 frameIndex,
				rcSharedFrameBufPtr& userFrameBuf,
				rcSharedFrameBufPtr*& cacheFrameBufPtr,
				const uint32 dToken);

  vfVideoCacheStatus cacheInsert(uint32 frameIndex,
  ...
  vfVideoCachePrefetchUnit*            _prefetchThread;
   ...
   */
  static map<uint32, vfVideoCache*>  _activeCachesItoP;
  static map<vfVideoCache*, uint32>  _activeCachesPtoI;
...
{
public:
	void operator() (vfVideoCache* vc)
	{
		if (vc != 0)
			vfVideoCache::vfVideoCacheDtor (vc);
	}
};

typedef boost::shared_ptr<vfVideoCache> shared_video_cache;

#define _shared_video_cache_create(name,_1,_2,_3,_4,_5,_6) sharedVideoCache name (vfVideoCache::vfVideoCacheCtor((_1), (_2), (_3), (_4), (_5), (_6) , (cc)), VideoCacheReleaser ())
