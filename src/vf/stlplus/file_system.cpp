...
#endif
  // chk whether the path is absolute or relative and discard the leading / if absolute
  if (i < spec.size() && is_separator(spec[i]))
  {
    m_relative = false;
    i++;
...
bool file_specification::initialise_file(const std::string& spec)
{
  ...
  while (--i)
  {
    if (is_separator(spec[i]))
      break;
#ifdef MSWINDOWS
    // on windows you can say a:fred.txt so the colon separates the path from the filename
    else if (i == 1 && spec[i] == ':')
      break;
#endif
  }
...
static std::string full_path(const std::string& root, const std::string& path)
{
  ...
  // decompose the root path and chk whether it is relative
  file_specification rootspec;
...
{
  // first chk whether the command is already a path and chk whether it exists
