/****************************************************************************
** Meta object code from reading C++ file 'rc_timeline.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../include/visUI/rc_timeline.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'rc_timeline.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_rcTimeline[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       7,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      17,   12,   11,   11, 0x0a,
      41,   12,   11,   11, 0x0a,
      77,   71,   11,   11, 0x0a,
     118,  108,   11,   11, 0x0a,
     155,   11,   11,   11, 0x0a,
     186,   11,   11,   11, 0x0a,
     202,   11,   11,   11, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_rcTimeline[] = {
    "rcTimeline\0\0time\0updateTime(rcTimestamp)\0"
    "updateCursorTime(rcTimestamp)\0state\0"
    "updateState(rcExperimentState)\0start,end\0"
    "updateRange(rcTimestamp,rcTimestamp)\0"
    "updateScale(rcResultScaleMode)\0"
    "updateDisplay()\0updateSettings()\0"
};

void rcTimeline::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        rcTimeline *_t = static_cast<rcTimeline *>(_o);
        switch (_id) {
        case 0: _t->updateTime((*reinterpret_cast< const rcTimestamp(*)>(_a[1]))); break;
        case 1: _t->updateCursorTime((*reinterpret_cast< const rcTimestamp(*)>(_a[1]))); break;
        case 2: _t->updateState((*reinterpret_cast< rcExperimentState(*)>(_a[1]))); break;
        case 3: _t->updateRange((*reinterpret_cast< const rcTimestamp(*)>(_a[1])),(*reinterpret_cast< const rcTimestamp(*)>(_a[2]))); break;
        case 4: _t->updateScale((*reinterpret_cast< rcResultScaleMode(*)>(_a[1]))); break;
        case 5: _t->updateDisplay(); break;
        case 6: _t->updateSettings(); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData rcTimeline::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject rcTimeline::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_rcTimeline,
      qt_meta_data_rcTimeline, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &rcTimeline::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *rcTimeline::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *rcTimeline::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_rcTimeline))
        return static_cast<void*>(const_cast< rcTimeline*>(this));
    return QWidget::qt_metacast(_clname);
}

int rcTimeline::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 7)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 7;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
