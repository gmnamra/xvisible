...
int rcCmdLine::splitLine(int argc, char **argv)
{
   ...
   {
      // if it's a switch, start a new rcCmdLine
      if (isSwitch(argv[i]))
      {
         curParam = argv[i];
         ...
         // add the rcCmdParam to 'this'
         pair<rcCmdLine::iterator, bool> res = insert(rcCmdLine::value_type(curParam, cmd));
      ...
      else
      {
         ...
         // ...let's add it
 	      rcCmdLine::iterator theIterator;
...
bool rcCmdLine::isSwitch(const char *pParam)
{
   if (pParam==NULL)
      return false;
  ...
  rmAssert (iIdx >= 0);
   ...
   if (hasSwitch(pSwitch))
   {
      ...
      theIterator = find(pSwitch);
	   if (theIterator!=end())
      {
	if ((*theIterator).second.m_strings.size() > (uint32) (iIdx))
         {
