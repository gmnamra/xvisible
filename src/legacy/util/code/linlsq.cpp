...
void LLSQ::remove(double x, double y) {          // delete an element
	assert (total_weight > 0.0);                       // illegal
...
// Returns the x,y means as an rc2Fvector
rc2Fvector LLSQ::mean_point() const {
  if (total_weight > 0.0) {
    return rc2Fvector(sigx / total_weight, sigy / total_weight);
  } else {
    return rc2Fvector(0.0f, 0.0f);
  }
}
...
double LLSQ::rms_orth(const rc2Fvector &dir) const {
	rc2Fvector v = dir.perpendicular ();
	v = v.unit ();
